...
class ProxyPool:
    ...
    def _looks_like_proxy_url(self, url: str) -> bool:
        ...
        只把 socks 协议和包含端口号但不含路径的 URL 视为代理地址；
        http(s) URL 默认当作 API 端点（代理池获取地址），除非明显是代理格式。
        """
        if url.startswith(("sock5://", "sock5h://", "socks5://", "socks5h://")):
            return True
        # http(s) URL 仅在 host:port 无路径时视为代理
        from urllib.parse import urlparse

        try:
            parsed = urlparse(url)
            # 有路径的 http(s) 认为是 API，例如 http://api.example.com/get_proxy
