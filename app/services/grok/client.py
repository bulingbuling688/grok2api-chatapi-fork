...
from app.services.grok.console_client import ConsoleClient
...
class GrokClient:
    ...
    async def openai_to_grok(request: dict):
        ...
        # Console.x.ai models use the Responses API with the same SSO token pool.
        info = Models.get_model_info(model)
        if info.get("is_console_model"):
            return await ConsoleClient.openai_to_console(request)
