...
class PostCreateManager:
    ...
    @staticmethod
    async def create(
        file_id: str, file_uri: str, auth_token: str
            ...
            for outer_retry in range(MAX_OUTER_RETRY + 1):  # +1 确保实际重试3次
                ...
                while retry_403_count <= max_403_retries:
                    ...
                    if retry_403_count > 0 and proxy_pool.enabled:
                        logger.info(
                            f"[PostCreate] 403重试 {retry_403_count}/{max_403_retries}，刷新代理..."
                        )
                    ...
                    async with AsyncSession() as session:
                        ...
                        if response.status_code == 403 and proxy_pool.enabled:
                            ...
                            if retry_403_count <= max_403_retries:
                                logger.warning(
                                    f"[PostCreate] 遇到403错误，正在重试 ({retry_403_count}/{max_403_retries})..."
                                )
                            ...
                            # 内层重试全部失败
                            logger.error(
                                f"[PostCreate] 403错误，已重试{retry_403_count - 1}次，放弃"
                            )
                        ...
                        if response.status_code in retry_codes:
                            if outer_retry < MAX_OUTER_RETRY:
                                ...
                                logger.warning(
                                    f"[PostCreate] 遇到{response.status_code}错误，外层重试 ({outer_retry + 1}/{MAX_OUTER_RETRY})，等待{delay}s..."
                                )
                                await asyncio.sleep(delay)
                                break  # 跳出内层循环，进入外层重试
                            else:
                                logger.error(
                                    f"[PostCreate] {response.status_code}错误，已重试{outer_retry}次，放弃"
                                )
                                raise GrokApiException(
                                    f"创建失败: {response.status_code}错误",
                                    "CREATE_ERROR",
                                )
