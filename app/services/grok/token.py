...
COOLDOWN_429_WITH_QUOTA = 90  # 429+有额度短冷却（秒）
...
class GrokTokenManager:
    ...
    def __init__(self):
        ...
        self._round_robin_cursors: Dict[str, int] = {}  # pool/field -> 下次轮询位置
    ...
    async def get_token(self, model: str) -> str:
        ...
        if self._extract_sso(jwt):
            return jwt
        return f"sso-rw={jwt};sso={jwt}"

    async def available_token_count(self, model: str) -> int:
        """获取当前模型可用Token数量。"""
        await self._reload_if_needed()

        current_time = time.time() * 1000

        def count_available(tokens: Dict[str, Any], field: str) -> int:
            count = 0
        ...
        if model == "grok-4-heavy":
            return count_available(super_tokens, "heavyremainingQueries")

        return count_available(normal_tokens, "remainingQueries") + count_available(
            super_tokens, "remainingQueries"
        )
    ...
    async def select_token(self, model: str) -> str:
        ...
        def select_round_robin(
            tokens: Dict[str, Any], field: str, cursor_name: str
            ...
            cursor = self._round_robin_cursors.get(cursor_name, 0) % len(available)
            token_key, remaining = available[cursor]
            self._round_robin_cursors[cursor_name] = (cursor + 1) % len(available)
        ...
        if model == "grok-4-heavy":
            ...
            token_key, remaining = select_round_robin(super_tokens, field, "ssoSuper:heavy")
        else:
            field = "remainingQueries"
            token_key, remaining = select_round_robin(normal_tokens, field, "ssoNormal:regular")
            if token_key is None:
                token_key, remaining = select_round_robin(super_tokens, field, "ssoSuper:regular")
    ...
    async def record_failure(self, auth_token: str, status: int, msg: str) -> None:
        ...
        try:
            ...
            if status == 429:
                data["lastRateLimitTime"] = int(time.time() * 1000)
                data["lastRateLimitReason"] = f"{status}: {msg}"
                logger.warning(f"[Token] 速率限制: {sso[:10]}... 原因: {msg}")
                self._mark_dirty()
                return
        ...
        try:
            ...
            if status_code == 429:
                ...
                if remaining > 0 or remaining == -1:
                    cooldown_seconds = self._rate_limit_cooldown_seconds(data)
                    cooldown_until = time.time() + cooldown_seconds
                    logger.info(
                        f"[Token] 429冷却(有额度): {sso[:10]}... 冷却{cooldown_seconds}秒"
                    )
                else:
                    # 无额度：冷却10小时
    ...
    @staticmethod
    def _rate_limit_cooldown_seconds(data: Dict[str, Any]) -> int:
        reason = str(data.get("lastRateLimitReason") or data.get("lastFailureReason") or "")

        minute_match = re.search(r"Requests per Minute \(actual/limit\):\s*(\d+)\s*/\s*(\d+)", reason)
        if minute_match:
            actual = int(minute_match.group(1))
            limit = max(1, int(minute_match.group(2)))
            overflow = max(0, actual - limit)
            return min(180, max(70, int((overflow / limit) * 60) + 70))

        second_match = re.search(r"Requests per Second \(actual/limit\):\s*(\d+)\s*/\s*(\d+)", reason)
        if second_match:
            return 10

        return COOLDOWN_429_WITH_QUOTA
